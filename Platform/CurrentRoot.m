//
//  [Vendored] CurrentRoot.m
//  WanWo / Platform
//
//  出处：逐字 vendored 自 ish-arm64 原版 app/CurrentRoot.m
//  （repos/ish-arm64-master/app/CurrentRoot.m，与 Vendor/ish/app/CurrentRoot.m
//  字节一致；09 决策 #6 复用范围）。仅添加本出处注释，其余原样。
//
//  依赖头文件解析：`#include "kernel/calls.h"`、`#include "fs/path.h"`
//  经 project.yml HEADER_SEARCH_PATHS（Vendor/include/ish 与 Vendor/ish 两根）
//  命中 Vendor/ish/kernel/calls.h、Vendor/ish/fs/path.h（kernel/calls.h 内部
//  再引入 kernel/fs.h（generic_open/generic_mkdirat/generic_rmdirat）与
//  fs/fd.h（struct fd/fd_close/AT_PWD 定义于 fs/path.h）），与 ISHKernel.m
//  同一套 include 路径，无需新增搜索路径。
//
//  ERR-019：补齐 rootfs overlay 补丁链——ISHKernel.m boot 流程在
//  createDeviceNodes 之后调用 FsApplyOverlay()，把 RootfsPatch.bundle 内
//  fetch-polyfill.js / wasm-polyfill.js 写入 guest /lib/（fakefs 持久化，
//  /ish/overlay-version 版本管理，装新 ipa 首次 boot 自动应用）。
//
//  平台分支说明：iOS 构建未定义 ISH_LINUX（project.yml 仅定义
//  GUEST_ARM64/ISH_INTERNAL），故走 `#if !ISH_LINUX` 的 guest-fs 读写分支，
//  LinuxInterop.h 分支不参与编译。
//
