...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/NotificationOffload.m
//   全文 499 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②notification_offload_register()：
...
static int cmd_schedule(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (actionSpec.length) {
        ...
        NSString *categoryId = [@"wanwo.cli." stringByAppendingString:
    ...
    if (!granted) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"schedule",
                                             ...
                                             @"Notification permission not granted. "
                                              "To grant access, open Settings > Notifications > WanWo "
...
void notification_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-notification", notification_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-notification");
        NSLog(@"WanWoOffload: apple-notification handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-notification handler (err=%d)", err);
    }
}
