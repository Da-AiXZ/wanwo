...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/SpeakOffload.m
//   全文 407 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②speak_offload_register()：
...
void speak_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-speak", speak_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-speak");
        NSLog(@"WanWoOffload: apple-speak handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-speak handler (err=%d)", err);
    }
}
