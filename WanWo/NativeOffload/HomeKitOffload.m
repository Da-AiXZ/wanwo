...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/HomeKitOffload.m
//   全文 784 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②homekit_offload_register()：
...
void homekit_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-homekit", homekit_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-homekit");
        NSLog(@"WanWoOffload: apple-homekit handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-homekit handler (err=%d)", err);
    }
}
