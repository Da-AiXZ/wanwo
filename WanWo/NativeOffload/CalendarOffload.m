...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/CalendarOffload.m
//   全文 1723 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②calendar_offload_register()：
...
static BOOL reminders_location_auth(NSString **errMsg) {
    ...
    if (errMsg) {
        *errMsg = @"Location access is required for location-based reminders. "
                   ...
                   "Location Services and enable WanWo. "
...
void calendar_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-calendar", calendar_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-calendar");
        NSLog(@"WanWoOffload: apple-calendar handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-calendar handler (err=%d)", err);
    }
}
