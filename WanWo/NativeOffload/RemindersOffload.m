...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/RemindersOffload.m
//   全文 163 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②reminders_offload_register()：
...
void reminders_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-reminders", reminders_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-reminders");
        NSLog(@"WanWoOffload: apple-reminders handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-reminders handler (err=%d)", err);
    }
}
