...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/MediaOffload.m
//   全文 513 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②media_offload_register()：
...
void media_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-media", media_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-media");
        NSLog(@"WanWoOffload: apple-media handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-media handler (err=%d)", err);
    }
}
