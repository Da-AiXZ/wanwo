...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/MapsOffload.m
//   全文 457 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②maps_offload_register()：
...
void maps_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-maps", maps_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-maps");
        NSLog(@"WanWoOffload: apple-maps handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-maps handler (err=%d)", err);
    }
}
