...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/HealthKitOffload.m
//   全文 4275 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②healthkit_offload_register()：
...
static BOOL requestHealthKitAccess(NSSet<HKObjectType *> *readTypes,
    ...
    if (!granted && outError) {
        ...
        *outError = [NSString stringWithFormat:
            @"%@. To grant access, open Settings > Health > Data Access & Devices > WanWo "
...
static NSString *const kHKOffloadErrorDomain = @"WanWoHealthKitOffload";
    ...
    if (!saveWithAuthRetry(bpCorrelation, sysType, &saveErr)) {
        ...
        if (authDenied) {
            msg = [NSString stringWithFormat:
                @"%@. Open Settings > Health > Data Access & Devices > WanWo "
...
void healthkit_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-healthkit", healthkit_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-healthkit");
        NSLog(@"WanWoOffload: apple-healthkit handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-healthkit handler (err=%d)", err);
    }
}
