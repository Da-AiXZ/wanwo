...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/WeatherOffload.m
//   全文 277 行，语义 1:1（B1c 批）】
//  `apple-weather` 原生 offload handler。Subcommands: current, hourly,
//  minute, daily, alerts, report。WeatherKit 为 Swift-only 框架，ObjC handler
//  经 WeatherOffloadBridge（Swift，随本文件 vendored）发异步 WeatherKit 请求
//  并回传 NSDictionary。
//  适配点（简报 B1c，共五处，其余逐行 1:1）：
//   1. 文件头注释 MinisApp → WanWo；
//   2. #import "Minis-Swift.h" → #import "WanWo-Swift.h"（target 改名）；
//   3. 定位拒绝文案 "enable Minis" → "enable WanWo"（品牌改名）；
//   4. weather_offload_register()：native_offload_add_handler →
//      wanwo_offload_register_checked（权限门控 trampoline，10-design:818
//      v2——OpenMinis 原件直连内核注册）；
//   5. NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"。
//
//  WeatherKit entitlement（com.apple.developer.weatherkit）已随本批补入
//  WanWo.entitlements（OpenMinis Minis.entitlements 原样）；签发依赖开发者
//  账号在 App ID 上启用 WeatherKit capability（评估见 B1c 报告）。
//
...
void weather_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-weather", weather_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-weather");
        NSLog(@"WanWoOffload: apple-weather handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-weather handler (err=%d)", err);
    }
}
