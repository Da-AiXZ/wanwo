...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/VisionOffload.m
//   全文 797 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②vision_offload_register()：
...
static NSString *const HELP_TEXT =
    @"apple-vision - Image analysis using the Vision framework\n"
     ...
     "  Supports images in /var/wanwo/attachments/ and other accessible paths.\n"
     "\n"
     "EXAMPLES:\n"
     "  apple-vision ocr /var/wanwo/attachments/photo.jpg\n"
     "  apple-vision ocr /var/wanwo/attachments/doc.png --lang zh-Hans,en --level fast\n"
     "  apple-vision barcode /var/wanwo/attachments/qr.png --compact -q\n"
     "  apple-vision classify /var/wanwo/attachments/photo.jpg --limit 5\n"
     "  apple-vision analyze /var/wanwo/attachments/photo.jpg\n"
...
void vision_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-vision", vision_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-vision");
        NSLog(@"WanWoOffload: apple-vision handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-vision handler (err=%d)", err);
    }
}
