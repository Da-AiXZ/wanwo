...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/PhotosOffload.m
//   全文 1387 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②photos_offload_register()：
...
static NSString *const HELP_TEXT =
    @"apple-photos - Query and manage the iOS photo library\n"
     ...
     "  export        Export a photo/video to /var/wanwo/offloads/\n"
     ...
     "                       Accepts guest paths (/var/wanwo/attachments/a.jpg),\n"
     ...
     "                       Accepts guest paths (/var/wanwo/...), paths relative\n"
     ...
     "  apple-photos import /var/wanwo/offloads/pic.jpg\n"
     "  apple-photos import --path /var/wanwo/offloads/pic.jpg\n"
     "  apple-photos import --path /var/wanwo/offloads/pic.jpg --album-name \"My Album\"\n"
     "  apple-photos save --path /var/wanwo/offloads/clip.mov --album <album-id>\n"
...
static int cmd_export(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    NSString *guestDir = @"/var/wanwo/offloads";
...
//   1. Guest-absolute: "/var/wanwo/attachments/foo.jpg" → mapped into fakefs data dir.
...
void photos_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-photos", photos_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-photos");
        NSLog(@"WanWoOffload: apple-photos handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-photos handler (err=%d)", err);
    }
}
