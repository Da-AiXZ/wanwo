...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/BluetoothOffload.m
//   全文 1110 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②bluetooth_offload_register()：
...
void bluetooth_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-bluetooth", bluetooth_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-bluetooth");
        NSLog(@"WanWoOffload: apple-bluetooth handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-bluetooth handler (err=%d)", err);
    }
}
