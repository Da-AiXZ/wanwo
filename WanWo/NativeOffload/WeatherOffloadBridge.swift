...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/WeatherOffloadBridge.swift
//   全文 209 行，语义 1:1（B1c 批）】
//  WeatherKit Swift bridge，供 WeatherOffload.m 调用。WeatherKit 无 ObjC API；
//  本类把天气数据以 NSDictionary 形态暴露给 ObjC handler。
//  适配点：仅文件头注释 MinisApp → WanWo；其余逐行 1:1（本桥零 App 层依赖——
//  仅 WeatherKit + CoreLocation，故直连 vendored）。
//
