...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/AlarmOffloadBridge.swift
//   全文 477 行，语义 1:1（B1c 批）】
//  AlarmKit Swift bridge，供 AlarmOffload.m 调用（AlarmKit 为 Swift-only）。
//  适配点（其余逐行 1:1）：
//   1. 文件头注释 MinisApp → WanWo；
//   2. MinisAlarmMetadata → WanWoAlarmMetadata（品牌改名，内部类型）；
//   3. AlarmLabelStore 载体目录 Library/MinisChat/ → Library/WanWo/
//      （§十三.8 minis→wanwo 路径改名纪律）；注释里 Minis UI 名同步改名。
//  版本降级路径与原件同款：整个文件包在 `#if canImport(AlarmKit)` 内——
//  SDK<26 时本文件编译为空，AlarmOffload.m 的引用点同在其版本 #if 块内，
//  运行时 iOS<26 由 @available + NOT_AVAILABLE envelope 收敛。
//
...
@available(iOS 26.0, *)
nonisolated struct WanWoAlarmMetadata: AlarmMetadata {}
...
@objc public class AlarmOffloadBridge: NSObject {
    ...
    // the label visible in the WanWo alarm list (label row renders
    // `alarm.label` when non-empty).
    //
    // Backed by a tiny SQLite table (AlarmLabelStore) in the same WanWo/
    // folder as other native DBs — O(1) keyed reads/writes instead of
    ...
    @objc public static func scheduleAlarm(
        ...
        Task {
            do {
                ...
                let attributes = AlarmAttributes<WanWoAlarmMetadata>(
            ...
            do {
                ...
                for await alarms in manager.alarmUpdates {
                    ...
                    for alarm in alarms {
                        ...
                        // only; the list row hides the label row when empty.
...
///
/// Mirrors the native-SQLite style used by WanWo's own SQLite surfaces
/// (`SessionDatabase` / `wanwo-index.sqlite3`): raw `SQLite3` (no ORM), WAL
/// journal, `synchronous = NORMAL`, in the shared `Library/WanWo/` folder.
/// The connection is opened once with FULLMUTEX (serialized threading) and
/// reused; an `NSLock` additionally guards the multi-statement prune
/// transaction. Callers reach it via the process-wide `shared` singleton.
private final class AlarmLabelStore {
    ...
    private static func defaultURL() -> URL {
        ...
        let base = library.appendingPathComponent("WanWo", isDirectory: true)
