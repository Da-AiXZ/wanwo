...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/FFmpegOffload.m
//   全文 514 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；
//   ②ffmpeg_offload_register()：native_offload_add_handler→
//     wanwo_offload_register_checked（权限门控 trampoline，10-design:818 v2）；
//   ③NSLog 前缀 "NativeOffloads:"→"WanWoOffload:"；
//   ④编译期依赖守卫（简报 B1b ffmpeg 特别处理授权）：FFmpeg 框架本体
//     本批不引入 CI——用 __has_include(<FFmpeg/FFmpeg.h>) 守卫，无框架时
//     编译"降级路径"：handler 返回 NOFF_ERR_NOT_AVAILABLE envelope +
//     NOFF_EXIT_NOT_AVAILABLE，并补 noff_ensure_guest_stub 使 `ffmpeg`
//     可被 PATH 解析（否则命令未找到，错误语义劣化）。有框架时走原逻辑。
//   注：原文件的 "[FFmpegOffload]" NSLog 模块标签保留原样（内容日志，
//   非 "NativeOffloads:" 品牌前缀，不在授权适配点内）。】
//
...
// ── WanWo M6.1 增（简报 B1b ffmpeg 特别处理）：编译期依赖守卫 ──
// FFmpeg.framework 本体（deps/ffmpeg-patch 构建链）本批不引入 CI。
// 无框架时整个框架依赖面（FFmpeg.h + fftools 全局 extern + stdio redirect）
// 不可用，必须整体守卫；降级路径只保留注册 + NOT_AVAILABLE envelope。
#if __has_include(<FFmpeg/FFmpeg.h>)
#define WANWO_FFMPEG_AVAILABLE 1
#endif

#ifdef WANWO_FFMPEG_AVAILABLE
...
#else  // !WANWO_FFMPEG_AVAILABLE

// ── WanWo M6.1 增：降级路径（简报 B1b ffmpeg 特别处理授权）──
// 无 FFmpeg.framework 时编译本分支：handler 以统一 envelope 自报不可用
// （NOFF_ERR_NOT_AVAILABLE + NOFF_EXIT_NOT_AVAILABLE），不进模拟器、
// 不改 envelope 词汇。--compact/-q flag 解析从简——无框架本就无转码
// 产物，标准 envelope 形态即可。
static int ffmpeg_handler(int argc, char **argv,
                          int stdin_fd, int stdout_fd, int stderr_fd) {
    (void)argc; (void)argv; (void)stdin_fd;
    NSDictionary *err = noff_json_error(@"ffmpeg", @"run", NOFF_ERR_NOT_AVAILABLE,
        @"ffmpeg is unavailable in this build: FFmpeg.framework is not linked.");
    noff_emit_json(stdout_fd, err, NO, NO);
    return NOFF_EXIT_NOT_AVAILABLE;
}

#endif  // WANWO_FFMPEG_AVAILABLE
...
void ffmpeg_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("ffmpeg", ffmpeg_handler);
    if (err == 0) {
        NSLog(@"WanWoOffload: ffmpeg handler registered");
#ifdef WANWO_FFMPEG_AVAILABLE
        ...
        if (native_offload_set_abort_handler("ffmpeg", ffmpeg_abort_requested) == 0)
            NSLog(@"WanWoOffload: ffmpeg abort handler registered");
        else
            NSLog(@"WanWoOffload: failed to register ffmpeg abort handler");
#else
        // 降级路径：OpenMinis 原件不建 guest stub（其 rootfs 自带 ARM ffmpeg
        // 可执行文件供 execve 命中 basename）。WanWo 无框架时无该二进制，
        // 不建 stub 则 `ffmpeg` 直接 command not found——错误语义劣化。
        // 补 stub 使 PATH 解析命中，由 handler 回 NOT_AVAILABLE envelope。
        noff_ensure_guest_stub("/usr/local/bin/ffmpeg");
#endif
    } else {
        NSLog(@"WanWoOffload: failed to register ffmpeg handler (err=%d)", err);
    }
}
