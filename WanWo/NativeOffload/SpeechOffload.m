...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/SpeechOffload.m
//   全文 504 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②speech_offload_register()：
...
static NSString *const HELP_TEXT =
    @"apple-speech - Speech recognition (audio → text)\n"
     ...
     "  apple-speech transcribe --source /var/wanwo/attachments/meeting.m4a\n"
...
static int cmd_transcribe(int argc, char **argv, int stdout_fd, BOOL compact, BOOL quiet) {
    ...
    if (!micSource) {
        ...
        if ([normalized isEqualToString:@"audio-file"] || [normalized isEqualToString:@"audio_file"]) {
            NSDictionary *err = noff_json_error(TOOL_NAME, @"transcribe",
                                                 NOFF_ERR_INVALID_ARGS,
                                                 @"When --source is an audio file, pass the file path directly (e.g. --source /var/wanwo/attachments/audio.m4a).");
            noff_emit_json(stdout_fd, err, compact, quiet);
            return NOFF_EXIT_INVALID_ARGS;
        }
    ...
    if (!micSource) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (recordingFormat.channelCount == 0 || recordingFormat.sampleRate <= 0) {
                recognitionError = [NSError errorWithDomain:@"WanWoSpeech" code:-3 userInfo:@{
            ...
            if (!tapInstalled) {
                recognitionError = [NSError errorWithDomain:@"WanWoSpeech" code:-4 userInfo:@{
...
void speech_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-speech", speech_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-speech");
        NSLog(@"WanWoOffload: apple-speech handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-speech handler (err=%d)", err);
    }
}
