...
//  WanWo
//
//  Native offload handler for `wanwo-browser-use`.
...
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/BrowserUseOffload.m
//   全文 357 行，语义 1:1（B2 批）】适配点：
//   1. 文件头注释 MinisApp → WanWo；
//   2. #import "Minis-Swift.h" → #import "WanWo-Swift.h"（target 改名）；
//   3. TOOL_NAME/帮助全文/stub 路径 minis-browser-use → wanwo-browser-use；
//      输出键 minis_url/fetched_minis_url → wanwo_url/fetched_wanwo_url；
//      资源引用 minis://browser/ → wanwo://browser/；注释 minis-config → wanwo-config；
//   4. 路径 /var/minis/browser/ → /var/wanwo/browser/（=WanWoPaths.browserLinuxDir）；
//   5. NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"；
//   6. browser_use_offload_register()：native_offload_add_handler →
//      wanwo_offload_register_checked（权限门控 trampoline，10-design:818 v2；
//      同 WeatherOffload.m B1c 惯例，并显式 #import "WanWoOffloadGate.h"）。
//
...
#elif __has_include("WanWoApp-Swift.h")
#import "WanWoApp-Swift.h"
#endif

static NSString *const TOOL_NAME = @"wanwo-browser-use";

static NSString *const HELP_TEXT =
    @"wanwo-browser-use - Drive the in-app WebView from the shell\n"
     "\n"
     "USAGE:\n"
     "  wanwo-browser-use <action> [options]\n"
     "  wanwo-browser-use --json '<json>'\n"
     "  wanwo-browser-use --help\n"
     "\n"
     "ACTIONS:\n"
     "  navigate        --url <url>\n"
     ...
     "                   /var/wanwo/browser/ and referenced via image_path +\n"
     "                   wanwo_url, so shells don't get flooded with base64.\n"
     ...
     "                      /var/wanwo/browser/screenshot_<ms>.jpg\n"
     "    wanwo_url         wanwo://browser/<filename> — stable reference for\n"
     ...
     "                      /var/wanwo/browser/\n"
     "    fetched_wanwo_url wanwo://browser/<filename> for the download\n"
     "\n"
     "EXAMPLES:\n"
     "  wanwo-browser-use navigate --url https://example.com\n"
     "  wanwo-browser-use screenshot\n"
     "  wanwo-browser-use screenshot --full-page\n"
     "  wanwo-browser-use click --selector '.btn-primary'\n"
     "  wanwo-browser-use type --selector 'input[name=q]' --text 'hello'\n"
     "  wanwo-browser-use execute_js --script 'return document.title'\n"
     "  wanwo-browser-use --json '{\"action\":\"navigate\",\"url\":\"https://x.com\"}'\n";
...
static NSDictionary *buildInputJson(int argc, char **argv, NSString **errOut) {
    ...
    // (mirrors `wanwo-config set --file`). Parse failures are surfaced as an
...
void browser_use_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("wanwo-browser-use", browser_use_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/wanwo-browser-use");
        NSLog(@"WanWoOffload: wanwo-browser-use handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register wanwo-browser-use handler (err=%d)", err);
    }
}
