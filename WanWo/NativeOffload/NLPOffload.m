...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/NLPOffload.m
//   全文 515 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②nlp_offload_register()：
...
void nlp_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-nlp", nlp_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-nlp");
        NSLog(@"WanWoOffload: apple-nlp handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-nlp handler (err=%d)", err);
    }
}
