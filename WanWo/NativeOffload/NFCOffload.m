...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/NFCOffload.m
//   全文 2152 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②nfc_offload_register()：
...
//
//  Entitlement file (WanWo.entitlements) declares:
...
void nfc_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-nfc", nfc_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-nfc");
        NSLog(@"WanWoOffload: apple-nfc handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-nfc handler (err=%d)", err);
    }
}
