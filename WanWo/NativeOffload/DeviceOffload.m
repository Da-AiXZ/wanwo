...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/DeviceOffload.m
//   全文 273 行，语义 1:1（bypass 档样板）】
//  `apple-device` 原生 offload handler。Subcommands: info, battery, storage
//  适配点（简报 B1a ⑤，共三处，其余逐行 1:1）：
//   1. 文件头注释 MinisApp → WanWo；
//   2. device_offload_register()：native_offload_add_handler →
//      wanwo_offload_register_checked（权限门控 trampoline，10-design:818
//      v2——OpenMinis 原件直连内核注册）；
//   3. NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"。
//
...
void device_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-device", device_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-device");
        NSLog(@"WanWoOffload: apple-device handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-device handler (err=%d)", err);
    }
}
