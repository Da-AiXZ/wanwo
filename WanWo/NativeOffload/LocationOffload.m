...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/LocationOffload.m
//   全文 318 行，语义 1:1。适配点（简报 B1b，其余逐行 1:1）：
//   ①头注释 MinisApp→WanWo；②location_offload_register()：
...
void location_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-location", location_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-location");
        NSLog(@"WanWoOffload: apple-location handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-location handler (err=%d)", err);
    }
}
