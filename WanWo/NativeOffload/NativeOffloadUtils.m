...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/NativeOffloadUtils.m
//   全文，语义 1:1】共享工具面实现。
//  适配点（简报 B1a ①）：NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"
//  （仅 noff_emit_json 序列化异常一处日志；其余逐行 1:1）。
//
...
void noff_emit_json(int fd, NSDictionary *dict, BOOL compact, BOOL quiet) {
    ...
    @try {
        ...
        // 【万我 M6.1 适配】NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"
        NSLog(@"WanWoOffload: JSON serialization exception: %@", e.reason);
    }
    if (!json) {
...
NSString *_Nullable noff_resolve_host_path(NSString *guestPath) {
    ...
    // 【万我核对注】WanWo 的 rootfs 同样装在 Documents/alpine-rootfs
    // （RootfsInstaller.swift:45），data 根换算 1:1 沿用。
