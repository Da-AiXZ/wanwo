...
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/NativeOffloads/ClipboardOffload.m
//   全文 322 行，语义 1:1（askOnce 档样板）】
//  `apple-clipboard` 原生 offload handler。Subcommands: get, set, clear, status
//  适配点（简报 B1a ⑤，共四处，其余逐行 1:1）：
//   1. 文件头注释 MinisApp → WanWo；
//   2. HELP_TEXT 示例路径 /var/minis/attachments → /var/wanwo/attachments
//      （§十三.8 路径改名纪律）；
//   3. clipboard_offload_register()：native_offload_add_handler →
//      wanwo_offload_register_checked（权限门控 trampoline，10-design:818
//      v2——OpenMinis 原件直连内核注册）；
//   4. NSLog 前缀 "NativeOffloads:" → "WanWoOffload:"。
//
...
static NSString *const HELP_TEXT =
    @"apple-clipboard - Read and write the iOS clipboard\n"
     ...
     "  apple-clipboard get --image /var/wanwo/attachments/clipboard_image.png\n"
     "  apple-clipboard set --text \"Hello World\"\n"
     "  apple-clipboard set --image /var/wanwo/attachments/chart.png\n"
...
void clipboard_offload_register(void) {
    ...
    int err = wanwo_offload_register_checked("apple-clipboard", clipboard_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/apple-clipboard");
        NSLog(@"WanWoOffload: apple-clipboard handler registered");
    } else {
        NSLog(@"WanWoOffload: failed to register apple-clipboard handler (err=%d)", err);
    }
}
