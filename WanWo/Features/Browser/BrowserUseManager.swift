//
//  BrowserUseManager.swift
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/Agent/BrowserUse/BrowserUseManager.swift，全文语义 1:1（B2 批）】
...
//    5. RootfsManager.shared → RootfsInstaller.shared（WanWo ISHRuntime 同语义件改名）；
//    6. AIChatViewModel.minisWorkspacePersistentDir → WanWoPaths.sessionPersistentDir
//       (bucket:"workspace")（WanWo 等价物直连，会话四桶存储形态）；
//    7. WanwoURLSchemeHandler.resolveWanwoURL 为降级桩——wanwo:// 资源 URL 解析体系
//       属 B3（M6.5 scheme/深链批），本批行为上只保 http/https 可用（简报 §3 口径），
//       scheme handler 注册/分发/mime 服务代码路径全保留；
//    8. origin 策略落点为本批新增（设计强制，OpenMinis 无此面）——语义源=codex
//       analysis/06-codex-gap5:117-124 default_origin_policy，判定本体居
//       OriginPolicy.swift：access=WKNavigationAction 决策处、downloads=两处
//       .download 决策处、uploads=引擎无 runOpenPanel 实现（原件同款，结构性 deny）。
//
...
final class BrowserUseManager: NSObject, ObservableObject {
    ...
    /// Shared scheme handler for `wanwo://` URLs across all browser tabs
    /// and in-app previews that embed a WKWebView.
    static let sharedWanwoSchemeHandler = WanwoURLSchemeHandler()
    ...
    private static func makeWebView(profile: UserAgentProfile,
        ...
        config.setURLSchemeHandler(Self.sharedWanwoSchemeHandler, forURLScheme: "wanwo")

        // Bridge JS window.print() to the native iOS print dialog. Injected on
        // every page (wanwo:// and external http/https) at document start so a
    ...
    nonisolated static let printMessageHandlerName = "wanwoPrint"
    ...
    private func presentPrintDialog() {
        ...
        printInfo.jobName = pageTitle.isEmpty ? "WanWo" : pageTitle
    ...
    private func navigate(to urlString: String?) async throws -> BrowserActionResult {
        ...
        // Only allow HTTP(S) and wanwo:// schemes for direct navigation
        let scheme = url.scheme?.lowercased() ?? ""
        if !["http", "https", "wanwo"].contains(scheme) {
            return .error("Cannot navigate to \(scheme):// URLs. Only http://, https://, and wanwo:// are supported.")
        }
        ...
        if let current = currentURL, let navURL = URL(string: current),
    ...
    private func getCookies(keywords: [String]?, fuzzy: Bool) async -> BrowserActionResult {
        ...
        let offloadsDir = RootfsInstaller.shared.dataPath
            .appendingPathComponent("var/wanwo/offloads", isDirectory: true)
        ...
        let envGuestPath = "/var/wanwo/offloads/\(envFilename)"
    ...
    static func wrapExecuteJSReturn(_ script: String) -> String {
        ...
        """
        const __wanwoNormalize = (function () {
            const MAX_DEPTH = 32;
        ...
        const __wanwoResult = await (async () => {
        \(script)
        })();
        return __wanwoNormalize(__wanwoResult);
        """
    }
...
extension BrowserUseManager: WKNavigationDelegate {
    ...
    nonisolated func webView(
        ...
        if navigationAction.shouldPerformDownload {
            // 【WanWo origin 策略 · downloads 维】决策落点（新增面，OpenMinis
            // 原件直通）。缺省档 downloads=ask → 走 OriginPolicy.askHandler 审批缝
            // （browser_use 工具调用期内接线；无缝/无工具上下文=用户手动浏览
            // → fail closed 取消，登记报告待拍板）。decisionHandler 允许延迟
            // 回调——async 审批后单次回决。
            let anchorURL = navigationAction.request.url
            Task { @MainActor [weak self] in
                let allowed = await OriginPolicy.shared.authorizeDownloads(
                    for: anchorURL?.absoluteString)
                if !allowed {
                    self?.resumePendingNavigationWithError(
                        "Download blocked by origin policy (downloads=ask/deny) for \(anchorURL?.host ?? "unknown origin").")
                }
                decisionHandler(allowed ? .download : .cancel, preferences)
            }
            return
        }
            ...
            Task { @MainActor [weak self] in
                ...
                if let cont = self.navigationContinuation {
                    ...
                    cont.resume(throwing: URLError(.unsupportedURL, userInfo: [
                        NSLocalizedDescriptionKey: "Blocked navigation to \(scheme):// — only http(s) and wanwo:// are allowed in the browser."
                    ]))
                }
            }
            return
        ...
        // 【WanWo origin 策略 · access 维】决策落点（新增面，OpenMinis 原件无——
        // 设计强制，语义源=codex default_origin_policy）。缺省档 access=allow
        // → 纯直通零行为差；deny 取消导航并回注 continuation。同步判定
        // （access 无 ask 执行路径——审批面向 downloads/uploads 副作用维）。
        if let url = navigationAction.request.url {
            let accessAllowed = MainActor.assumeIsolated {
                OriginPolicy.shared.allowsAccess(for: url.absoluteString)
            }
            if !accessAllowed {
                decisionHandler(.cancel, preferences)
                Task { @MainActor [weak self] in
                    self?.resumePendingNavigationWithError(
                        "Navigation blocked by origin policy (access=deny) for \(url.host ?? url.absoluteString).")
                }
                return
            }
        }
    ...
    nonisolated func webView(
        ...
        if let http = navigationResponse.response as? HTTPURLResponse,
           ...
           disposition.contains("attachment") {
            // 【WanWo origin 策略 · downloads 维】决策落点（同 anchor 下转分支）。
            let responseURL = navigationResponse.response.url
            Task { @MainActor [weak self] in
                let allowed = await OriginPolicy.shared.authorizeDownloads(
                    for: responseURL?.absoluteString)
                if !allowed {
                    self?.resumePendingNavigationWithError(
                        "Download blocked by origin policy (downloads=ask/deny) for \(responseURL?.host ?? "unknown origin").")
                }
        ...
        if !navigationResponse.canShowMIMEType {
            // 【WanWo origin 策略 · downloads 维】同上（MIME 下转分支）。
            let responseURL = navigationResponse.response.url
            Task { @MainActor [weak self] in
                let allowed = await OriginPolicy.shared.authorizeDownloads(
                    for: responseURL?.absoluteString)
    ...
    /// 【WanWo origin 策略 · 配套】把被政策取消的导航/下转回注给挂起的
    /// navigate() continuation——否则 agent 侧会挂满 30s 超时（blocked-scheme
    /// 分支同语义；无挂起 continuation 时为 no-op）。
    @MainActor
    fileprivate func resumePendingNavigationWithError(_ message: String) {
        isLoading = false
        if let cont = navigationContinuation {
            navigationContinuation = nil
            cont.resume(throwing: URLError(.badURL, userInfo: [
...
extension BrowserUseManager: WKDownloadDelegate {
    ...
    /// 【WanWo 适配 · 降级】OpenMinis=ViewModelCache → AIChatViewModel.appendSystemInfo
    /// （往所属会话对话流插一条 ephemeral 系统提示）。WanWo 无聊天 VM 缓存层
    /// （AIChatViewModel 未随 M6 移植，会话 UI 面为 M9 范围）——降级为
    /// AppLogger 记录（agent 仍可经 download 报告/工具结果看到下载终态；
    /// 对话内系统提示挂账 M9 接入，登记报告）。
    private func notifySessionChat(sid: String, text: String, icon: String) {
        logger.info("[SessionNotice] sid=\(sid.prefix(8)) icon=\(icon) \(text)")
    }
...
// MARK: - WanwoURLSchemeHandler

/// Handles `wanwo://` URLs in WKWebView by resolving them to local files.
/// Supports `wanwo://workspace/file.html`, `wanwo://shared/...`, etc.
final class WanwoURLSchemeHandler: NSObject, WKURLSchemeHandler {
    private let logger = AppLogger(category: "WanwoScheme")

    func webView(_ webView: WKWebView, start urlSchemeTask: any WKURLSchemeTask) {
        let url = urlSchemeTask.request.url!
        logger.info("[WanwoScheme] start \(url.absoluteString)")

        // 【WanWo 适配 · 降级 · B3 接线】OpenMinis 原件=AIChatViewModel.resolveMinisURL(url)
        // （minis://workspace/shared 资源 URL 解析体系，OpenMinis App 层）。
        // WanWo 的 wanwo:// 资源 URL 体系属 B3（M6.5 wanwo:// scheme/深链批）；
        // 本批保留 scheme handler 代码路径（注册/分发/mime 服务全链在位），
        // 解析端降级为恒 nil —— 行为上 wanwo:// 资源 URL 暂不可用，仅 http/https
        // 可用（m6-scope-brief §3 派单口径）。B3 落 resolveWanwoURL 实体即可接线。
        guard let fileURL = Self.resolveWanwoURL(url) else {
            logger.warning("[WanwoScheme] not found (B3 接线前恒 nil): \(url.absoluteString)")
            urlSchemeTask.didFailWithError(URLError(.fileDoesNotExist))
            return
        }

        guard let data = try? Data(contentsOf: fileURL) else {
            logger.warning("[WanwoScheme] read failed: \(fileURL.path)")
