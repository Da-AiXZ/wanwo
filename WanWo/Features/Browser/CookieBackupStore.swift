//
//  CookieBackupStore.swift
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/Agent/BrowserUse/CookieBackupStore.swift，全文语义 1:1（B2 批）】
...
///
/// Mitigation: before every browser_use / wanwo-browser-use action (throttled
/// to once per 60s) we
...
final class CookieBackupStore {
    ...
    /// 【WanWo 测试缝 · B2】备份目录注入点（派单【验证】"cookie backup 存储
    /// 往返（可注入存储）"）。生产恒 nil → backupDirURL 走 Application Support
    /// 默认；单测注入临时目录做文件级往返。OpenMinis 原件无此缝——仅分派点
    /// 一处，存储格式零改动。
    var backupDirectoryOverride: URL?
    ...
    // 【WanWo 测试缝 · B2】private→internal（存储往返单测可达；字段零改动）。
    struct BackupCookie: Codable {
        var name: String
        var value: String
        var domain: String
        var path: String
    ...
    private var backupDirURL: URL {
        // 【WanWo 测试缝 · B2】注入目录优先（单测临时目录往返）。
        if let override = backupDirectoryOverride { return override }
    ...
    /// `#HttpOnly_`.
    // 【WanWo 测试缝 · B2】private→internal（同上）。
    static func renderNetscape(_ cookies: [BackupCookie]) -> String {
        var lines = ["# Netscape HTTP Cookie File", "# Written by WanWo CookieBackupStore — do not edit while the app runs", ""]
    ...
    // 【WanWo 测试缝 · B2】private→internal（同上）。
    static func parseNetscape(_ text: String) -> [BackupCookie] {
