//
//  BrowserTabPool.swift
//  WanWo
//
//  【vendored 复用 · 源=OpenMinis src/ios/Agent/BrowserUse/BrowserTabPool.swift，全文语义 1:1（B2 批）】
...
final class BrowserTabPool: ObservableObject {
    ...
    /// grace window. This is the `wanwo-browser-use` CLI path: a human/script
    ...
    private static func storeURL(for sessionId: String) -> URL {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            .appendingPathComponent("WanWo/browser_tabs", isDirectory: true)
