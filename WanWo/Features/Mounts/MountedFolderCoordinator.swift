...
//  WanWo
//
//  【vendored 原件 · 出处 OpenMinis `src/ios/Views/Settings/MountedFolderCoordinator.swift`（GPL v3）】
//  10-design 附录 A.5 / :248-348 NSFileCoordinator 四处（读/写/删/替换）全生命周期。
//
...
//
//  【万我适配清单】：
//    1. AppLogger 直连（同名同构件）。
//    2. AppLocalized(...) → String(localized: ...)（B2 降级登记先例）。
//    3. AIChatViewModel.minisMountsLinuxDir → WanWoPaths.mountsLinuxDir。
//
...
enum MountedFolderCoordinator {
    ...
    /// Checks both the fakefs path (/var/wanwo/mounts/<name>/...) and
    ...
    /// read-only at add time. Used by file tools to pre-reject writes with a
    /// friendly message instead of failing deep in the call stack with EACCES.
    /// 【事故纪律⑤】只读预拒写。
    ...
    /// Returns true if `linuxPath` (a `/var/wanwo/mounts/<name>/...` path from
    /// the iSH namespace) is under a mount entry that is effectively read-only.
    /// Compares by mount name under `/var/wanwo/mounts/`, so it's immune to the
    ...
    static func prefetchIfNeeded(at directory: URL) {
        // no-op（原件原样保留——语义本身即 no-op，非未实现桩）
    }
    ...
    /// from the main thread. Callers dispatch reads/writes through background
    /// queues; if a future caller needs main-thread safety they should hop to
    /// a background queue first.
