...
//  WanWo
//
//  【vendored 原件 · 出处 OpenMinis `src/ios/iSH/RootfsManager.swift`（GPL v3）】
//  10-design 附录 A.2 / §四 ISHRuntime 映射 / §十三.8 改名纪律。
//  逐文件 vendored：自带纯 Swift ZIP 解析器（ZIPArchive）、.arch 架构标签、
//  move-aside 删除健壮性（[T-rootfs-undeletable-blocks-install]）、
//  reset 毒标记（[T-rootfs-reset-terminal-crash]）、meta.db 注册全套 ——
//  全部事故修复逐行保留，禁止按映射表转译重写。
//  万我命名适配（仅产品字符串）：RootfsManager→RootfsInstaller、
//  minis-mcp-cli→wanwo-mcp-cli、/var/minis→/var/wanwo。
//
...
class RootfsInstaller {
    ...
    private let logger = AppLogger(category: "RootfsInstaller")

    /// [T-mcp-cli-help-and-readonly-ios] Guest path of the bundled wanwo-mcp-cli
    /// Python lib. Its files are mounted read-only (see applyDefaultMountOverlay).
    /// 【万我适配】原值为 /usr/local/lib/minis-mcp-cli。
    private static let mcpCliLibDir = "/usr/local/lib/wanwo-mcp-cli"
    private static let mcpCliLibPrefix = "/usr/local/lib/wanwo-mcp-cli/"
    ...
    func installIfNeeded() throws {
        ...
        // Pre-create /var/wanwo/ shared directory structure
        // 【万我适配】原值为 var/minis/{...}。
        let wanwoSubdirs = ["var/wanwo/attachments", "var/wanwo/offloads", "var/wanwo/workspace", "var/wanwo/skills", "var/wanwo/shared"]
        for subdir in wanwoSubdirs {
    ...
    func applyDefaultMountOverlay() {
        ...
        while let fileURL = enumerator.nextObject() as? URL {
            ...
            do {
                ...
                // like /usr/local/bin/wanwo-open are always executable.
                // 【万我适配】原注释为 minis-open。
                ...
                // [T-mcp-cli-help-and-readonly-ios] The bundled wanwo-mcp-cli
        ...
        // wanwo-mcp-cli lib FILES read-only (0o444), but ensureParentDirsInMetaDB
...
private class ZIPArchive {
    /// Own logger: this is a separate type from RootfsInstaller, and its failures
