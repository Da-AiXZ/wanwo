//
//  ShellCommandRingBuffer.swift
//  WanWo
//
//  【vendored 原件 · 出处 OpenMinis `src/ios/Agent/ISH/ShellCommandRingBuffer.swift`（GPL v3）】
//  10-design 附录 A.2 / §四 ISHRuntime 映射（同名文件）。
//  环形缓冲配平事故修复逐行保留：
//    [T-ios-shellring-counter-leak] didAbort 恒定终态 + liveStarts 精确集合
//    防重复终止窃取 _runningCount 扣减（既不锁高也不锁低）。
//  无任何外部依赖，除本头注外与原件逐行一致。
//
