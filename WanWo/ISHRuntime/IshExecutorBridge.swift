//
//  IshExecutorBridge.swift
//  WanWo
//
//  【vendored 原件 · 出处 OpenMinis `src/ios/Agent/ISH/ISHExecutionCoordinator.swift`（GPL v3）】
//  10-design 附录 A.2 / §四 ISHRuntime 映射 / §5.4 执行缝（F026）/ §十三.8 改名纪律。
//  actor ISHExecutionCoordinator → IshExecutorBridge（万我命名适配，§四 v2.2 省略
//  IshShellExecutorShim —— 原件已覆盖 ObjC 门面职责，不再自加中间层）。
//  保留原件全部事故修复与注释（注释是纪律的一部分，§十三.8）：
//    · 每命令一 fork /bin/sh + stdin 注入（exec 0</dev/null 包装 + [T-heredoc-trailing-newline] 尾换行）
//    · [T-ish-continuation-double-resume] 四路竞争单点 claimResume
//    · [T-ish-thread-leak] 超时杀整进程组 + [T-ish-shell-timeout-leak] finalizeTimedOutPid
//    · [T-shell-stop-blocked-by-actor] nonisolated 静态活 PID 表快照
//  万我命名适配：MinisFsRouter→FsContextRouter、RootfsManager→RootfsInstaller、
//  EnvVarStore→WanWoEnvStore（薄 stub，见该文件头注）、
//  AIChatViewModel.minis* 静态路径常量→WanWoPaths.*（/var/minis→/var/wanwo）。
//
...
actor IshExecutorBridge {
    static let shared = IshExecutorBridge()
    ...
    /// handled by FsContextRouter's hook, not by bind_mount, so they don't
    /// need to be re-mounted on session switch.
    private var staticMountsInitialized: Set<String> = []

    /// Maps linux mount paths (e.g. "/var/wanwo/memory") to their host
    ...
    /// FsContextRouter to resolve their host paths.
    ...
    /// routed by FsContextRouter's hook now (no bind-mount swap needed), so
    ...
    func mountForSession(_ sessionId: String) {
        ...
        _ = FsContextRouter.shared.context(for: sessionId)
        ensureStaticMountsInitialized(for: sessionId)
        mountedSessionId = sessionId
    }
    ...
    private func ensureStaticMountsInitialized(for sessionId: String) {
        ...
        // they're hooked by FsContextRouter.
    ...
    /// Resolve a Linux path under /var/wanwo/ to its current host URL.
    /// Tries the static bind-mount table first (memory/skills/shared +
    /// external folders), then falls back to FsContextRouter for the per-
    ...
    func hostURL(for linuxPath: String, sessionId: String? = nil) -> URL? {
        ...
        if let sid = sessionId ?? mountedSessionId,
           let url = FsContextRouter.shared.hostURL(forGuest: linuxPath, sid: sid) {
            return url
        }
        return nil
    }
    ...
    /// Bind-mount a session's wanwo directories into iSH-visible /var/wanwo/.
    /// 【万我适配】原注释为 /var/minis/；挂载目录常量经 WanWoPaths 提供。
    private func performMount(_ sid: String) {
        ...
        for (idx, (persistDir, linuxDir)) in subdirs.enumerated() {
            ...
            if let hostDir = resolveHostPath(linuxDir) {
                ...
                if lstatOk && (lstatBuf.st_mode & S_IFMT) == S_IFLNK {
                    ...
                    // Safety: only remove paths under data/var/wanwo/ to avoid accidentally
                    // deleting other fakefs directories (e.g. data/dev/) due to path bugs.
                    // 【万我适配】原路径 data/var/minis/。
                    let dataVarWanwo = RootfsInstaller.shared.dataPath.appendingPathComponent("var/wanwo").path
                    guard hostDir.path.hasPrefix(dataVarWanwo) else {
                        logger.error("MOUNT [\(idx)] SAFETY: refusing removeItem outside var/wanwo: \(hostDir.path)")
                        break
                    }
        ...
        // them just like /var/wanwo/shared.
    ...
    private func resolveHostPath(_ linuxPath: String) -> URL? {
        ...
        return RootfsInstaller.shared.dataPath.appendingPathComponent(relative)
    }
