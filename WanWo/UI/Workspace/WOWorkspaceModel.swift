...
public struct WOSessionNode: Identifiable, Equatable {
    ...
    public var pendingKind: WOPendingKind? = nil

    public init(id: String, title: String?, blank: Bool, createdAt: Date, updatedAt: Date) {
        self.id = id
        self.title = title
        self.blank = blank
...
public enum WOPendingKind: Equatable {
    case approval, planReview, question
}

// MARK: - 分组节点（tree.ts GroupNode）

public struct WOGroupNode: Identifiable, Equatable {
    /// '' = 未分组桶（tree.ts UNGROUPED_KEY）
    public let key: String
    public let workspaceId: String?
    public let label: String
    public let createdAt: Date?
    public let sessions: [WOSessionNode]
    public var containsCurrent: Bool = false
...
public enum WOGroupBy: String, CaseIterable {
    case workspace, flat
}

public enum WOOrderBy: String, CaseIterable {
    case manual, updated
}

/// 视图偏好持久化（dsh persist 'dsh.workspace.view.v5' → 万我键名独立 v1）
@MainActor
public final class WOWorkspaceViewStore: ObservableObject {
    @AppStorage("wo.workspace.groupBy.v1") public private(set) var groupByRaw: String = WOGroupBy.workspace.rawValue
    @AppStorage("wo.workspace.orderBy.v1") public private(set) var orderByRaw: String = WOOrderBy.updated.rawValue
    /// 组展开账本（瞬态；5+ 折叠的展开态）
    @Published public private(set) var groupExpansion: Set<String> = []

    public var groupBy: WOGroupBy { WOGroupBy(rawValue: groupByRaw) ?? .workspace }
    public var orderBy: WOOrderBy { WOOrderBy(rawValue: orderByRaw) ?? .updated }

    public init() {}

    public func setGroupBy(_ v: WOGroupBy) { groupByRaw = v.rawValue }
    public func setOrderBy(_ v: WOOrderBy) { orderByRaw = v.rawValue }

    /// dsh actions.setGroupExpanded——显式写展开态（旧侧栏 :972 同语义）
    public func setGroupExpanded(_ key: String, _ expanded: Bool) {
        if expanded { groupExpansion.insert(key) } else { groupExpansion.remove(key) }
    }
    /// 展开态查询：账本无记录 = 展开（dsh 默认展开；折叠是显式动作）
    public func isExpanded(_ key: String) -> Bool { !groupExpansion.contains(key) }
}

// MARK: - 派生器（tree.ts deriveGroups / deriveFlat，行 719-721）

public enum WOWorkspaceTreeDeriver {

    public static let ungroupedKey = ""
...
public enum WOSessionStatus: Equatable {
    ...
    public var dotState: WOStateDotState {
        switch self {
    ...
    public static func resolve(for node: WOSessionNode) -> WOSessionStatus {
...
public enum WOTimeLabel {
    public static func short(_ t: WORelativeTime) -> String {
        switch t.unit {
        case .now: return "刚刚"
    ...
    public static func rowLabel(updatedAt: Date, now: Date = Date()) -> String {
        short(WORelativeTimeBucket.relativeTime(at: updatedAt, now: now))
    }

    /// hover 卡时间（now 裸「刚刚」，其余「{t}前」）
    public static func hoverLabel(updatedAt: Date, now: Date = Date()) -> String {
